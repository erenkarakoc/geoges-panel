-- 0029 — the helpers say which normalizer wrote them (TASK-0110, D-247, D-266).
--
-- The search row and its postings already carry the normalizer's version (0027); the bucket and
-- the vocabulary did not. Two versions could therefore share one bucket, and a count could mix
-- words folded by different rules. From here each helper row belongs to one version: reading
-- asks for the version in force, and a version left over from before is neither read nor
-- counted. Changing `core.fold_tr` or `core.search_words` means raising
-- `core.search_normalization_version()` and rebuilding; until the rebuild finishes the search
-- refuses to answer rather than answering from two rules at once.

alter table core.search_word_bucket
  add column normalization_version integer not null default 1
  constraint ck_search_word_bucket__normalization_version check (normalization_version > 0);
alter table core.search_word
  add column normalization_version integer not null default 1
  constraint ck_search_word__normalization_version check (normalization_version > 0);
...
alter table core.search_word_bucket add constraint pk_search_word_bucket
  primary key (word, record_type, scope_key, data_class, normalization_version);
alter table core.search_word drop constraint pk_search_word;
alter table core.search_word add constraint pk_search_word
  primary key (word, record_type, normalization_version);

-- Normally empty, like the row and posting indexes of 0027: a healthy index pays nothing here.
create index ix_search_word_bucket__old_normalization on core.search_word_bucket (record_type)
  where normalization_version <> 1;
create index ix_search_word__old_normalization on core.search_word (record_type)
  where normalization_version <> 1;

-- ---------------------------------------------------------------------------------------------
-- Upkeep: every helper row is written with the version in force
-- ---------------------------------------------------------------------------------------------
...
begin
  -- Out of every bucket this record is in but should not be: a word it lost, a move to another
  -- place or class, or a bucket an older normalizer wrote.
  update core.search_word_bucket b
   ...
   where b.search_document_ids operator(extensions.@>) array[p_search_document_id]
     and (b.record_type <> p_record_type or b.scope_key <> p_scope_key
          or b.data_class <> p_data_class or b.normalization_version <> version
          or not (b.word = any (p_words)));

  insert into core.search_word_bucket (word, record_type, scope_key, data_class,
                                       normalization_version, search_document_ids)
  select w, p_record_type, p_scope_key, p_data_class, version, array[p_search_document_id]
    from pg_catalog.unnest(p_words) as w
  on conflict (word, record_type, scope_key, data_class, normalization_version) do update
...
begin
  ...
  select row_id, w, p_record_type, p_site_id, p_project_id, p_owner_user_id,
    ...
    from pg_catalog.unnest(words) w;

  insert into core.search_word (word, record_type, normalization_version, record_count)
  select p.word, p.record_type, p.normalization_version, count(*)::int
   ...
   group by p.word, p.record_type, p.normalization_version
  on conflict (word, record_type, normalization_version) do update
   ...
   where w.record_type in (p_record_type, existing.record_type)
     ...
     and not exists (select from core.search_posting p
                      where p.word = w.word and p.record_type = w.record_type
                        and p.normalization_version = w.normalization_version);
...
begin
  ...
  with gone as (
    delete from core.search_posting p where p.search_row_id = removed.id
    returning p.word, p.normalization_version
  )
  update core.search_word w
     set record_count = greatest(0, w.record_count - 1)
   where w.record_type = removed.record_type
     and (w.word, w.normalization_version) in (select word, normalization_version from gone);
...
-- ---------------------------------------------------------------------------------------------
-- Reading: one version answers, never two
-- ---------------------------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------------------------
-- The integrity report knows about versions too
-- ---------------------------------------------------------------------------------------------
  ...
  with expected as materialized (
    ...
    select count(*) as n from expected_buckets e full join core.search_word_bucket b
      on b.word = e.word and b.record_type = e.record_type
        ...
        and b.normalization_version = e.normalization_version
    where e.ids is distinct from b.search_document_ids
  ), expected_vocabulary as (
    select word, record_type, normalization_version, count(*) as n
      from expected group by word, record_type, normalization_version
  ), vocabulary as (
    select count(*) as n from expected_vocabulary e full join core.search_word v
      on v.word = e.word and v.record_type = e.record_type
        and v.normalization_version = e.normalization_version
