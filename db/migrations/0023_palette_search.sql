-- TASK-0110: one database request for the palette, with six candidates per visible type.
-- SECURITY INVOKER keeps every result and spelling candidate under the caller's RLS.
create function core.search_palette(p_query text, p_types text[] default null) returns jsonb
...
begin
  ...
  if words is null then return jsonb_build_object('hits', hits, 'corrected', null, 'failed_types', failed); end if;
  ...
  if types is null then return jsonb_build_object('hits', hits, 'corrected', null, 'failed_types', failed); end if;
...
$$;
revoke all on function core.search_palette(text, text[]) from public;
grant execute on function core.search_palette(text, text[]) to geoges_app;
