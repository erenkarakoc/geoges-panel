-- TASK-0110: explicit normalizer version; existing rows were all generated with version 1.
-- Keep this version and the mismatch-index predicates in step when changing fold_tr/search_words.
create function core.search_normalization_version() returns integer
language sql immutable parallel safe set search_path = '' as $$ select 1 $$;
revoke all on function core.search_normalization_version() from public;
grant execute on function core.search_normalization_version() to geoges_app, geoges_worker;

alter table core.search_row add column normalization_version integer not null default 1
  constraint ck_search_row__normalization_version check (normalization_version > 0);
alter table core.search_posting add column normalization_version integer not null default 1
  constraint ck_search_posting__normalization_version check (normalization_version > 0);
alter table core.search_posting add column projection_version integer not null default 1
  constraint ck_search_posting__projection_version check (projection_version > 0);
update core.search_posting p set projection_version = r.projection_version
  from core.search_row r where r.id = p.search_row_id;

-- Normally empty: a valid large index does not pay for a full scan just to check versions.
create index ix_search_row__old_normalization on core.search_row (record_type)
  where normalization_version <> 1;
create index ix_search_posting__old_normalization on core.search_posting (record_type)
  where normalization_version <> 1;

-- RLS stays active. A hidden stale row must not even cause a different error for the caller.
create function core.assert_search_normalization(p_types text[] default null) returns void
language plpgsql stable security invoker set search_path = '' as $$
begin
  if exists (select from core.search_row
      where normalization_version <> core.search_normalization_version()
        and (p_types is null or record_type = any(p_types)))
    or exists (select from core.search_posting
      where normalization_version <> core.search_normalization_version()
        and (p_types is null or record_type = any(p_types))) then
    raise exception using errcode = 'P0001', message = 'search_normalization_mismatch';
  end if;
end
$$;
revoke all on function core.assert_search_normalization(text[]) from public;
grant execute on function core.assert_search_normalization(text[]) to geoges_app, geoges_worker;
...
create or replace function core.search_suggest(p_word text, p_types text[] default null) returns text
language plpgsql stable
set search_path = ''
as $$
declare result text;
