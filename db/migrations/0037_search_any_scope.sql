-- 0037 — one place is enough to find a record that names two (TASK-0110, D-270, OQ-034).
--
-- A search row names the place its record belongs to, and it may name two of them: a site and a
-- project. Until now only the first was judged — `coalesce(site, project)` — so a record naming
-- both was visible to the site's people and invisible to the project's, whatever their grants
-- said. The owner answered OQ-034 on 2026-09-23: authorisation for **any** one of a record's
-- places is enough (D-270).
--
-- Each place is judged whole, not mixed: a place must grant both the module's view and, for a
-- commercial or sensitive record, the data class. Someone who may view the site and may see
-- commercial only on the project therefore still does not see a commercial record of both —
-- neither place allows it on its own, and "any place is enough" is not "any combination is
-- enough". The narrower reading of the owner's answer is the one taken here, and it is written
-- down rather than assumed.
--
-- The data class rule, the own-record rule and every other check are unchanged, and nothing
-- outside search moves: this function is the search index's own access test.

create or replace function core.search_access_allows(p_access jsonb, p_module text, p_site uuid,
                                                     p_project uuid, p_owner uuid, p_class text)
returns boolean
language sql immutable parallel safe
set search_path = ''
as $$
  with places as (
    select 'site' as kind, p_site as id where p_site is not null
    union all
    select 'project', p_project where p_project is not null
    union all
    select 'company', null::uuid where p_site is null and p_project is null
  )
  select exists (
    select from places p
    where exists (
      select from jsonb_to_recordset(p_access -> 'grants')
        as g(permission_code text, scope_type text, scope_ids uuid[])
    ...
    and (p_class in ('general', 'internal') or exists (
      select from jsonb_to_recordset(p_access -> 'classes')
        as d(module text, scope_type text, scope_ids uuid[],
             can_see_commercial boolean, can_see_sensitive boolean)
...
-- The read-time helper check (0030) asks a different question and must keep asking it: a bucket
-- is kept under one place — the site when there is one — so it is readable only by that place's
-- people. Judging it with "any place" would make a record's bucket look missing to someone who
-- reaches the record through its other place, and report a healthy index as damaged. The check
-- now names the bucket's own place, so it judges what it is actually looking at.
...
begin
  ...
  if all_words_known then
    ...
    -- A word nobody has written: the suggestion has to be free to look everywhere (0032).
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
           ...
           coalesce(bool_or(
             core.search_access_allows(access, pg_catalog.split_part(x.record_type, '.', 1),
                                       x.site_id,
                                       case when x.site_id is null then x.project_id end,
                                       null, x.data_class)
    ...
    foreach kind in array coalesce(types, '{}') loop
      begin
        select coalesce(jsonb_agg(to_jsonb(r)), '[]'::jsonb),
               coalesce(bool_or(
                 core.search_access_allows(access, s.record_schema, s.site_id,
                                           case when s.site_id is null then s.project_id end,
                                           null, s.data_class)
                 and (select count(distinct b.word) from core.search_word_bucket b
                       where b.word = any (checked)
