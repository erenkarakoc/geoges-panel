...
- **Gecikme ve ölü mektup** denetim kaydına yazılır ve `npm run jobs:status` ile görünür; ayrıca `tsk.system-watch` işi (beş dakikada bir) ölü mektup varsa ya da kuyruk 15 dakikadan fazla gerideyse sahip katmanına kritik bildirim ve tek bir sistem sorunu görevi açar, sorun geçince görevi kapatır (TASK-0108, D-263).
