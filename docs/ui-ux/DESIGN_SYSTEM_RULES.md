...
| 15c | Bölüm şeridinin kaydırma çubuğu | Tarayıcı varsayılanı (yer kaplar) | `scrollbar-width: none` ile gizlenir, dikey kaydırma kapatılır | Ayrılan çubuk şeridi 53px'e çıkarıp 44px'lik satırdan taşırıyordu. **2026-09-26 (sahip):** firma kartının sekme şeridi de aynı sebeple (seçili sekmenin alt çizgisi listenin bir piksel altında) boşuna dikey kaydırma çubuğu gösteriyordu; aynı çözüm orada da uygulandı | `context-bar.tsx`, `party-card.tsx` |
...
| 17a | Telefon gezinmesi | COSS'ta alt gezinme çubuğu yok; sidebar telefonda `Sheet` çekmecesi olur | `md` altında kabuk, içeriğin altında kendi alt çubuğunu basar (iş katmanı + ortada birincil eylem + Modüller); sidebar çekmecesi ve üstteki menü düğmesi bu genişlikte kalkar | D-069, REQ-NFR-008: günlük kullanılan ekranlar tek dokunuşta olmalı. Birincil eylem telefonda yalnız `+` ikonuyla, yuvarlak düğme olarak durur (sahip 2026-09-17); eylemin adı erişilebilir ad olarak kalır. **2026-09-23 (sahip):** çubuk sahada dar bulundu; girişler ve birincil düğme 44px yerine **64px**, simgeler 20px yerine 24px (birincil düğmenin artısı 28px). Ayrıca telefonda Görevler listesi tablo yerine kart basar (yan kaydırma parmakla kullanılmıyor). **2026-09-25:** üst çubuktaki "Ara veya git…" kutusu telefonda 44px (23 Eylül'de masaüstüyle aynı 36px bırakılmıştı; sahip yanındaki düğmelerle aynı boya getirilmesini istedi); firma listesi ve kartı da aynı kurala uyar — sekmeler, "Firma ekle" ve rol süzgeci telefonda 44px (`max-md:`), masaüstünde COSS boyutu kalır | `mobile-bottom-bar.tsx`, `primary-action.tsx`, `app-header.tsx` |
...
| 19a | Odak halkası rengi (açık tema) | `--ring` neutral-400; açık zeminde 2,25:1 | `--ring` neutral-600 (6,8:1); koyu tema değişmez. Uygulandı 2026-09-21 (TASK-0054). Ölçümde COSS'un temel katmanının, kendi odak stili olmayan öğelerde (düz bağlantılar) çizgiyi `--ring`'in %50'siyle çizdiği görüldü: yeni renkle bile 2,27:1. Açık temada temel katmandaki bu çizgi tam `--ring` rengine çekildi (6,8:1); bileşenlerin kendi sınıfları önde kalır | WCAG 2.2 AA 1.4.11, sahip kararı D-225 | `brand.css` |
| 19b | Akış şeması alanı (özgün element) | COSS'ta kutu-ok şeması bileşeni yok | Akış tasarımcısının ana alanı; adım soruları COSS `Sheet` / `Drawer` içinde. Kutular: türünün simgesi, türü, adı; en üstte akışın neyle başladığını söyleyen kesikli "Akış başlar" kutusu (tıklanınca akışın kendi soruları açılır). Oklar: uçlarında yön oku; geri dönen yol ("geri", "düzelt ve tekrar") kesikli çizgiyle kutuların sağından dolaşır. Yol adları ("evet", "onay", "ret") kendi çıkış noktasının altında durur, her çıkışın yatay çizgisi ayrı yükseklikte döner. "+" eklenecek adımın hemen üstündedir; bir adıma birden fazla yol geliyorsa tek "+" vardır ve yeni adım bütün gelen yolların önüne girer. Küçük harita yalnız 10'dan fazla adımda; telefonda şema okunur boyutta, başlangıçtan açılır. Yapıldı Phase 08, iyileştirme 2026-09-26 (sahip isteği) | Sahip onayı D-224; SCR-196 | `modules/wfl/ui/flow-canvas.tsx` |
| 19d | Dar ekranda Ctrl/Cmd+B | Kısayol, 768 px altında COSS'un kendi mobil çekmecesini açar | Dar ekranda kısayol proje katmanında durdurulur; telefon gezinmesi alt bant ve "Modüller" çekmecesidir. Kenar tutamağının adı ve ipucu "Menüyü aç veya kapat" | D-069; COSS çekmecesinin başlığı COSS dosyasında İngilizce sabit (ADR-011); TASK-0054 | `app-sidebar.tsx`, `sidebar-drag-rail.tsx` |
| 19c | Grafikler (özgün element) | COSS'ta grafik bileşeni yok | Dağılım çubuğu (`ShareBar`), yatay çubuk listesi (`BarList`), günlere göre sütun grafiği (`ColumnChart`); düz elemanlardan çizilir, kütüphane yok. Tek değerli ilerleme COSS `Meter` ile gösterilir. Kurallar: bu belgenin §14 bölümü ve `docs/ui-ux/CUSTOM_ELEMENTS.md`. Yapıldı 2026-09-26 | Sahip onayı D-066, D-226, D-297 | `platform/ui/chart/chart.tsx` |
| 20 | Sayfalama metinleri | `PaginationPrevious` / `PaginationNext` görünen metni "Previous" / "Next", kök `aria-label` "pagination" | Ortak `ListPagination`: aynı COSS parçaları (`Pagination`, `PaginationLink`, `PaginationEllipsis`, `Button`) partikül düzeniyle (`p-pagination-1…3`) birleştirilir; önceki/sonraki yalnız ok simgesidir, görünür metin yok (sahip isteği 2026-09-22); erişilebilir adlar "Sayfalama", "Önceki sayfa", "Sonraki sayfa", "Sayfa 3". Sayfa bağlantısı zinciri `PaginationLink` → `Button` → Next.js `Link` (`render` verilince `PaginationLink` stili işlenen öğeye bırakır; COSS partikülü `p-pagination-3` de `Button` işler); ilk ve son sayfada önceki/sonraki devre dışı `Button`. Telefonda sayı düğmeleri yerine "3 / 12". `PaginationEllipsis` değiştirilmeden kullanılır (metni görünmez ve ekran okuyucuya kapalı). Her liste bu bileşeni kullanır; COSS sayfalama parçaları doğrudan ekranlarda kullanılmaz | Türkçe arayüz (ADR-011) | `platform/ui/list/list-pagination.tsx` |
| 21 | Zildeki okunmamış sayacı | COSS'ta simge düğmesinin köşesine oturan sayaç kalıbı yok (`p-button-32` rozeti metnin yanına koyar) | COSS `Badge` (`size="sm"`) düğmenin sağ üst köşesine konumlandırılır: `absolute -end-1 -top-1 min-w-5`; 99'dan büyükse "99+"; ekran okuyucu sayıyı düğmenin adından duyar ("Bildirimler, 3 okunmamış") | Sayaç zil simgesinden ayrı okunmalı, düğme boyutu değişmemeli (SCR-015, REQ-TSK-009). Uygulandı 2026-09-22, TASK-0108 | `src/modules/tsk/ui/notification-bell.tsx` |
| 22 | Arama paletinin telefon görünümü | COSS Command penceresi sınırlı yükseklikte ve kenarlardan boşluklu | Aynı `CommandDialogPopup` 768 px altında tam ekran; yüksekliği görünür ekran alanını izler, COSS `Button` ile görünür kapatma eklenir, `CommandFooter` her genişlikte korunur. Sonuç satırına `gap-2` verilir: COSS satırında ikonla metin arasında boşluk yok, belge ikonu firma adına yapışıyordu (sahip, 2026-09-26). Kaynak COSS dosyaları değiştirilmez | SEARCH.md bölüm 3–5, TASK-0110; klavye açıldığında sonuçların kaydırılabilmesi | `src/platform/ui/app-shell/command-palette.tsx` |
| 23 | Grafik renkleri | COSS varsayılan `--chart-1…5`: açık temada turuncu, camgöbeği, koyu camgöbeği ve iki ayrı sarı (iki renk neredeyse aynı); koyu temada bambaşka beş renk | Tek bir renk takımı, iki zemine göre ayrı ayrı adımlanmış: mavi, turuncu, su yeşili, sarı, pembe. Sıra renk körlüğüne göre ölçüldü (bitişik renkler arasında ΔE ≥ 8,4). İlk renk marka mavisinin ailesinden. `globals.css`'e dokunulmadı; değerler `brand.css`'te | D-297: grafikler uygulamanın tasarımıyla uyumlu olsun (sahip 2026-09-26) | `platform/ui/theme/brand.css` |

Önceki bilinen fark giderildi (2026-09-21, TASK-0054): kenar tutamağı proje katmanından Türkçe ad alır (satır 19d); COSS'un İngilizce başlıklı mobil çekmecesi artık açılamaz. COSS dosyasına dokunulmadı.
...
## 14. Grafikler: genel kural (sahip kuralı, 2026-09-26, D-297)

Bir ekranda sayıyla anlatılan bir şey varsa ve grafik onu daha çabuk okutuyorsa, sayının yanında grafik de bulunur. Bu kural yeni ekranlar için de, yapılmış ekranlar için de geçerlidir.

| Bilginin türü | Grafik |
|---|---|
| Ne kadar iş yapıldı, ne kadarı kaldı (tek değer ve hedefi) | COSS `Meter` (ilerleme çubuğu), yanında "12 / 40" gibi rakam |
| Bir bütün nasıl bölünüyor (duruma göre duvarlar, görevler) | `ShareBar` |
| Kalem kalem karşılaştırma (tipe göre hedef, önceki ve yeni revizyon) | `BarList` |
| Günden güne ne oldu (çalışmalar, olaylar, üretim) | `ColumnChart`; sürekli bir ölçü (fiyat, oran) gelirse çizgi grafik eklenir |
| Tek bir sayı, karşılaştırılacak bir şey yok | Grafik değil, sayının kendisi |

- Grafik rakamın yerine geçmez: her grafik rakamlarını yazıyla da gösterir (açıklama, çubuğun yanındaki değer, ekran okuyucu tablosu).
- Seriler yalnız renkle ayrılmaz; her serinin adı yazılıdır. Renkler sabit sırayla verilir, beşten fazla seri olmaz.
- Renkler tema değişkenlerinden gelir (`--chart-1…5`, bu belgenin §4.1 bölümü, satır 23); iki temada da ayrı ayrı ölçülmüştür. Durum renkleri (başarı, uyarı, hata) seri rengi olarak kullanılmaz.
- İnce çizgiler, 4 px yuvarlak uçlar, birbirine değen parçalar arasında 2 px boşluk; üzerine gelince veya klavyeyle odaklanınca ipucu çıkar.
- Kullanıcının göremediği veri grafikte de yoktur; olmayan veri uydurulmaz (üretim verisi gelmeden "yapılan iş" çizilmez).
- Her yeni ekranın öz denetiminde sorulur: "Bu ekranda grafikle daha iyi okunacak bir sayı var mı?" (`docs/standards/QUALITY_GATES.md`, madde 13).

**Geriye dönük uygulama (2026-09-26).** Grafik eklenen ekranlar: proje kartı "Duvarlar ve hedefler" (duvarların durumu, tipe göre panel ve şerit hedefi), proje listesi ve şantiye listesi (tamamlanan duvar), revizyon sayfası (önceki ve yeni hedef), görevler (duruma göre), akış çalışma günlüğü (son 14 günün çalışmaları, sonuca göre), denetim kaydı (son 14 günün kayıtları), kullanıcılar (ikinci doğrulamanın durumu). Bilerek grafiksiz bırakılanlar ve nedeni: "Bugün" göstergeleri örnek veri gösteriyor, gerçek veri gelince grafik alır; şantiyenin günlük hedefleri, üretim verisi gelince (günlük saha kaydı) hedef ve gerçekleşen karşılaştırmasını alır; firma listesi, onay kuyruğu ve tanımlar ekranlarında grafikle daha iyi okunan bir sayı yok.
