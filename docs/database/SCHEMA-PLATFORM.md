...
**Kuruldu (TASK-0102, 0003 göçü, D-256).** Yukarıdaki kısıtlara ek olarak: sahip katmanı hiçbir yoldan daraltılamaz (rol, atama, yetki, veri sınıfı, istisna, hesap pasifleştirme); tam görünürlüklü rol yalnız tüm şirket kapsamıyla atanır; rol hiyerarşisinde döngü olamaz. Kayıt, yetki, atama ve istisna tabloları taşınabilirliğe göre ayrılır: rol, yetki tipi, rol-yetki ve rol-veri sınıfı `config:export` ile taşınır; kişi adı taşıyan atama, istisna, elle amir ve rol seçimi taşınmaz. Etkin yetki `iam.my_grants()` ve `iam.my_data_classes()` ile hesaplanır; modül politikaları `iam.has_company_scope`, `iam.scope_ids`, `iam.can_see` ve `iam.acting_role_valid` ile yazılır (`CONVENTIONS.md` bölüm 10). `iam.recovery_code`, `iam.login_attempt` ve `iam.session` TASK-0112'de, 0038 göçüyle kuruldu.

**0038 hesap güvenliği (TASK-0112, D-272).** Üç tablo da kimsenin girmiş olmadığı bir anda ya da kişinin kendisinin okumaması gereken şeyler için yazılır, bu yüzden uygulama onlara doğrudan erişmez: `iam.login_attempt` ve `iam.recovery_code` üzerinde uygulama rolüne **hiç yetki verilmez** (politika değil, yetkinin kendisi yok), `iam.session` yalnız `select` alır ve politikası kişiye kendi oturumlarını gösterir. Bütün yazma işleri 0038'in tanımlayıcı (definer) işlevlerindedir: `iam.login_lock`, `iam.note_login_attempt`, `iam.start_session`, `iam.use_session`, `iam.mark_session_second_factor`, `iam.revoke_session`, `iam.revoke_sessions`, `iam.issue_recovery_codes`, `iam.use_recovery_code`, `iam.recovery_codes_left`. Eşik ve süreler burada değil: tarihli kural olarak okunup parametre geçilir (D-257, tohum `0007_iam_rules.sql`). Kilit olayı denetim kaydına aktörsüz düşer, çünkü kilit kimse girmeden önce olur. Hesap `disabled` olduğunda tetikleyici aynı işlemde bütün oturumları iptal eder (REQ-IAM-006); ayrılış tarihi işi de aynı sütundan geçtiği için REQ-IAM-007 ikinci bir yol istemez. Oturum doğduğunda süresi geçmiş olamaz (`ck_session__expires`). **0040** ile `iam.note_second_factor(user, present)` geldi: `must_setup_2fa` artık sağlayıcıyı izliyor (faktör doğrulanınca temizleniyor, sıfırlamadan sonra yazılıyor) ve denetim kaydına `two_factor.enrolled` / `two_factor.reset` düşüyor. Çağırabilenler kişinin kendisi ve `iam.module.manage` yetkisi olan yönetici; kontrol politikada değil işlevin içinde. **0041** ile yöneticinin sıfırlaması outbox'a `two_factor.reset` olayı bırakıyor (bildirim TSK'nın işi; IAM onu kodda çağırmıyor). **0042** ile `iam.second_factor_required()` geldi: kişinin rollerinden biri yöneticinin listesindeyse (`iam.two-factor-roles` tarihli kuralı) ikinci faktör zorunlu; işlev yalnız `core.current_user_id()` için cevap verir, başkası hakkında sorulamaz. **0043** ile `iam.people_security()` geldi: asgari kullanıcı ekranının (D-273) ihtiyaç duyduğu kişi listesi ve erişim durumu. Kurtarma kodlarında uygulamanın hiç yetkisi, oturumlarda ise yalnız kendi satırlarını görme izni olduğu için ekran bunu tablolardan toplayamaz; işlev yalnız `iam.module.manage` taşıyana cevap verir ve **sayı** döner — kod karması ya da oturum kimliği değil.

**0039 güvenlik ayarları fonksiyonun kendisinde:** eşik ve süreler çağrıyla gelmez. `iam.security_number(anahtar, varsayılan)` tarihli kurallardan (`adm.rule_value`) okur ve kilit, oturum ve hareketsizlik fonksiyonları onu kendileri çağırır. İki sebep: çağrıyla gelen bir güvenlik eşiğini çağıran belirler, ve ayarı uygulama katmanında okumak `IAM → ADM` oku demek olurdu — `ADM → IAM` zaten varken bu bir döngü. IAM'in denetim olaylarında olduğu gibi, öteki platform şemasına SQL içinden uzanılır (MODULE_MAP, D-272). Tohumlanan değerler: 5 deneme, 15 dakika kilit, 30 gün oturum, 3 gün hareketsizlik.
...
| `adm.panel_type` | Panel tipi | `code`, `name`, `width_m`, `height_m`, `area_m2` (türetilmiş), `series`, `step`, `project_id`, `status` | m² en × boydan hesaplanır (ADM-K2). Komşu tip seri + kademeyle tutulur: aynı serideki kademe ±1, ±2 (REQ-SIT-019; tek `neighbour_type_id` "bir alt / bir üst"ü gösteremezdi, TASK-0121). Kod ve ölçü oluşturulduktan sonra değişmez |
| `adm.strip_type` | Şerit tipi | `code`, `name`, `width_mm`, `thickness_mm`, `hole_count`, `standard_lengths_m[]`, `project_id`, `status` | Kod ve kesit oluşturulduktan sonra değişmez; standart boylar düzenlenir |
| `adm.consumption_recipe` | Sarf reçetesi | `output_kind`, `panel_type_id`, `strip_type_id`, `per_unit`, `material_item_id`, `qty_per_unit`, `valid_from`, `project_id`, `reason` | Günlük kayıttaki öneri buradan (REQ-ADM-004). Satır hiç güncellenmez; tek okuma noktası `adm.recipe_lines` (proje > genel, tip > her tip, en son geçerli tarih) |
...
**0028 yardımcı bütünlüğü:** `core.search_integrity()` yalnız işletim bağlantısına açık, SECURITY INVOKER bir tam taramadır. Tek sorguda beklenen sözcükleri `search_row.search_text` üzerinden üretir; eşleme içeriği ve kapsam/sahip/sınıf/tür/sürüm alanları, kovaların tam sıralı kimlik dizileri ve sözlük sayıları bağımsız karşılaştırılır. Satırın normalleştirme sürümü ve katlanmış metni de denetlenir. Yalnız kategori bazında bigint sayımlar döner. `row_security=off` yetki yükseltmez; kısmi RLS görünümünün sağlıklı tam dizin sanılmasını engeller. EXECUTE yalnız işlemciye verilir. Yayın kapısı ve salt okunur `search:rebuild -- --check` aynı işlevi kullanır; kullanıcı arama yoluna tam tarama eklenmez. Kalıcı tablo/alan sayısı değişmez. Kova/sözlük sürüm ayrımı ve görünür kayıtlarla sınırlı okuma anı denetimi ayrı açık işlerdir.
...
**0029 yardımcı sürümleri:** `core.search_word` ve `core.search_word_bucket` artık `normalization_version` taşır ve birincil anahtarları bu sütunu içerir. Bakım işlevleri yalnız yürürlükteki sürüme yazar, kaydı yabancı sürümün kovasından çıkarır ve sayımı sürüm içinde tutar; okuma yolu ile `core.search_integrity()` sürüm bazında çalışır. Normalleştirici sürümü değişirse 0027'deki kısmi dizinlerin yüklemleri gibi bu tabloların kısmi dizin yüklemleri de güncellenmelidir. Kalıcı tablo sayısı değişmez.

**0030 okuma anı denetimi:** `core.search_palette`, döndürdüğü kayıtlar için (tür başına en çok altı) her sorgu sözcüğünün kovasında o kaydın bulunup bulunmadığına bakar ve cevabın tamamı için tek bir `helper_mismatch` bayrağı döner. Kişinin geniş erişimi kaydın kapsamını kapsamıyorsa (kaydı yalnız sahibi olduğu için görüyorsa) kova okunamaz ve yargı verilmez. Denetim anahtar okumasıdır, tam tarama değildir; cevabı değiştirmez, hata üretmez ve tarayıcıya çıkmaz. Var olup bir kimliği düşmüş kovanın kaydı gizlemesi bu yolla görülemez; o yön `core.search_integrity()` ve yayın kapısındadır. `core.search_records` imzası değişmez, kalıcı tablo/alan sayısı da değişmez.

**0036 yayına özel toplu yazıcı:** `core.index_search_row_bulk` satırı ve eşlemelerini yazar, kovaya ve sözlüğe hiç dokunmaz; yalnız `geoges_worker`'a açıktır ve **tek bir kayıt için asla çağrılmaz**. Sebebi ölçülmüştür: yayın her kaydı sıradan `core.index_search_row` ile yazarken, üç bin kimlik taşıyan bir kova eklenen her kimlik için baştan yazılıyordu ve bu emeğin tamamını yayının sonundaki genel yardımcı kurulumu zaten çöpe atıyordu. 20.000 kayıtlık yeniden kurma on dakikadan **29 saniyeye** indi (2.000 kayıt 4,7 sn). Tek başına gelen kayıt yine `core.index_search_row` ile yazılır; aramanın kayıtlarından bir an bile geri kalmamasını sağlayan budur (TASK-0110).

**0034-0035 hacim düzeltmeleri:** Eşleşme artık önce eşlemeler üzerinden sayılıyor (materyalize CTE) ve yalnız bütün sözcükleri taşıyan satırlar birincil anahtarla okunuyor; eskiden planlayıcı `core.search_row` tablosunun tamamını dolaşıp her satıra satır güvenliği uyguluyordu. `core.search_posting` politikası kaydın modülünü artık satırı okuyarak değil kendi `record_type` değerinin ilk parçasından alıyor; bunu güvenli kılan yeni kısıt `ck_search_row__record_type_module`, kayıt türünün ilk parçasının şema adına eşit olmasını zorunlu tutuyor (0021'den beri kova politikasının varsaydığı kural artık yazılı). Palet, sorgunun bütün sözcükleri dizinde varsa tür listesini hiç sormuyor. Kalıcı tablo sayısı değişmiyor, yeni sütun yok.

**0037 çoklu kapsam:** `core.search_access_allows` artık kaydın adını andığı yerlerin **her birine** ayrı ayrı bakıyor; birine yetki yetiyor (D-270, OQ-034). Eskiden `coalesce(site, project)` ile yalnız ilki yargılanıyordu, yani hem şantiye hem proje taşıyan bir kayıt projenin insanlarına görünmüyordu. Her yer bir bütün olarak değerlendirilir: bir yerin hem modül görme yetkisini hem de ticari/hassas sınıf iznini vermesi gerekir; ayrı yerlerden toplanan yarım yetkiler birleştirilmez. Kova tek bir yer altında tutulduğu için (şantiye varsa şantiye) okuma anı denetimi kovanın kendi yerine bakar; aksi halde kaydı diğer yerinden bulan kişiye sağlıklı dizin bozukmuş gibi görünürdü. Tablo, sütun ve izin değişikliği yoktur.
