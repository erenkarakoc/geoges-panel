...
CURRENT PHASE:       PHASE 09 Slice 1 — Projects, Sites, Daily Site Log (opened 2026-09-25, plan D-291); Phases 07 and 08 are built, their owner walks postponed or owed (D-278)
CURRENT SUBPHASE:    IMPLEMENT — TASK-0121 and TASK-0122 built (TESTING); TASK-0123 IMPLEMENTING, steps 1-4 of 5
                     done (migrations 0064-0065, 0068, 0071; D-292, D-295, D-298); next: step 5, the flow
                     link check, search and samples. 2026-09-26 also: designer names and removal (D-293),
                     known answers chosen (D-294), bank days for rates (D-296), charts as a rule (D-297),
                     the designer canvas made easier to read
CURRENT FEATURE:     —
...
PHASE 08 SO FAR:     TASK-0118 (capability catalog with its contract test, 35 published capabilities) and TASK-0117
                     (the engine: event/clock/threshold/manual triggers, all fourteen steps, windowed conditions,
                     scheduler-backed waits, branches as child runs, the dry run as the same loop with a sink that
                     writes nothing) are DONE. TASK-0119 is built in five steps on 2026-09-25: the flows list
                     (SCR-195, minimal by D-283) with "Yeni akış"; the designer (SCR-196) with the canvas — the one
                     custom element (D-224), the drawing library fetched only on this route, one tab stop with arrow
                     keys, the mini-map not rendered on a phone; a pure layout function and a pure editing module
                     with their own tests; every step's own questions whose choices come from the capability
                     catalogs rather than lists in the screen; error marks that are the schema's own findings;
                     automatic saving that never sends a draft the schema refuses; the dry run asked of the worker
                     and watched for (D-284, with the sample record honestly empty until Faz 09R); the publish
                     confirmation REQ-WFL-024 asks for; and the header's copy, close and version history. 355 unit
                     tests, 337 database tests, lint, types, format and build pass. What is left is not code: the
                     owner has to walk the screen (the dev server's session expired and passwords are the owner's
                     to type).
PHASE 08 ALSO:       TASK-0120 built 2026-09-25 in six steps: the approval's addressing rule and approvals that
                     belong to a group (0057), the approval centre's real queue with the sentence that says why
                     it is with you and a badge that agrees in three places (0058), the template register with
                     copy / reset / "a newer version exists" (0059) and the announcement that tells a copy's
                     owner once (0060), the working log (SCR-197), the "Yeni akışlar" tab, and the eight
                     end-to-end processes as chains of short flows — 27 templates, every link of every process,
                     each parsed with the engine's schema and dry-run in a test. The clock trigger learned "the
                     Nth of each month" on the way (D-286). 377 unit tests, 354 database tests, lint, types,
                     format and build pass. Left: the owner's walk of the queue, the designer, the templates
                     and the working log; and a note worth keeping — `npm run test:db` wants the dev server
                     stopped, because that server runs the job worker and competes for outbox deliveries.
STATUS:              IMPLEMENTING
...
2026-09-25 (latest): TASK-0122 built — the firm card (`crm.party`, migration 0062, the new `crm` schema): one card per tax number, a new role joins the existing card, similar names suggested past the words every firm name carries, contacts, custom values, events and search. Screens `/leads-clients/parties` and SCR-083; samples in `db/samples`. 396 unit tests, 12 new database tests; CI 36172765624 green (migrate, rollback, reapply). Owed: the owner's walk (the duplicate tax number was seen refused in the browser).
2026-09-26: the owner's findings on the firm card fixed (phone targets, the tab strip's needless vertical scrollbar plus an ESLint rule against it, search icon spacing); migration 0063 lets the search index's `core.` worker register, which had stopped the dev server. Next: TASK-0123's plan (T1) for the owner's approval.
