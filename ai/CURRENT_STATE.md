...
CURRENT FEATURE:     —
CURRENT TASK:        TASK-0110 site-wide search IMPLEMENTING: per-type list links, permission-revalidated recents, mobile fullscreen and transactional source rebuild implemented; small-fixture warm latency passes after 0025 (258 ms p95, previously 348 ms); 0027 adds row/posting normalization versions and controlled errors for visible stale versions (23 search DB tests pass including rollback/backfill; CI 35811145952 green; latest remote 56-row p95 255 ms). Real-source concurrency tests now cover publication commit/rollback, changes behind the scan cursor and source rollback; pending deliveries converge without duplicate results (9b6a9f9, CI 35830079315 green, 26 search tests). Pre-publication catch-up now captures a finite difference of visible outbox IDs and refreshes staged source projections, including low-ID late commits; 32 search scenarios are defined and passed across local full/targeted runs, 273 unit tests and build pass (73116da, CI 35832578935 green: 172 DB tests including 32 search tests before and after full rollback; two credential-dependent R2 tests skipped). Latest remote small-fixture warm p95 274 ms / max 278 ms. Multi-scope visibility awaits OQ-034; helper version parity, browser acceptance and production-scale performance/rebuild cost remain. TASK-0113 still IMPLEMENTING. Then TASK-0028, TASK-0112 and TASK-0111 (M1).
STATUS:              IMPLEMENTING
