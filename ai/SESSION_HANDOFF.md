...
- **REQ-EQP** — 21 requirements, D-155…D-162, DRAFT; scope §20.3/§21.3 corrected in place; REQ-SIT-003 gains an equipment section (D-162). Derived rule awaiting confirmation: grouped items expensed on purchase (D-158). Two answers went against the recommendation: no overhead allocation to projects (D-149, changes §15.1/§22.4 wording) and period close per site (D-154, with a derived rule for factory and office marked for confirmation).
...
1. Owner confirms or corrects REQ-EQP (incl. the D-158 derived rule); mark CONFIRMED.
2. Continue with HR, then CRM, following the same pattern (scope → open questions only → D-NNN → REQ file with catalog → glossary first → records → commit).
