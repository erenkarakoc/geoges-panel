...
1. Continue with QTE (ask the §6.3 overhead-markup question), following the same pattern (scope → open questions only → D-NNN → REQ file with catalog → glossary first → records → commit).
