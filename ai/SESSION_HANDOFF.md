...
- **REQ-MTG (8), REQ-DOC (10), REQ-SUP (5)** — D-199…D-201, DRAFT. Awaiting: derived rules REQ-MTG-008 (open decisions on the next agenda) and REQ-SUP-004 (resolved ticket closes itself).
...
1. Owner confirms or corrects REQ-MTG, REQ-DOC, REQ-SUP (incl. two derived rules); mark CONFIRMED.
2. Continue with STR, following the same pattern (scope → open questions only → D-NNN → REQ file with catalog → glossary first → records → commit).
