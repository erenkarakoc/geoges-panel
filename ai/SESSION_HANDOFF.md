...
- **REQ-STR** — 8 requirements, D-202…D-205, DRAFT. Awaiting: derived rules REQ-STR-001 (annual targets as PRF company target) and REQ-STR-002 (budget revision).
...
1. Owner confirms or corrects REQ-STR (incl. two derived rules); mark CONFIRMED.
2. Continue with RPT §34, then NFR, following the same pattern (scope → open questions only → D-NNN → REQ file with catalog → glossary first → records → commit).
