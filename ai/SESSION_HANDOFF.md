...
- **REQ-QHS** — 16 requirements, D-182…D-185, DRAFT. Awaiting: derived PPE paper-record rule (REQ-QHS-015); whether REQ-HR-002/-014 keep medical reports given D-183.
...
1. Owner confirms or corrects REQ-QHS and answers the REQ-HR health-data question; mark CONFIRMED.
2. Continue with PRF, following the same pattern (scope → open questions only → D-NNN → REQ file with catalog → glossary first → records → commit).
