...
- **Status:** `DONE` — owner approved the exit 2026-09-21. Seventeen spikes, every roadmap candidate covered: SPIKE-01–11 and 13–17 PASS, SPIKE-12 PASS under the owner's cold-start exception (D-248 / CHG-008). No ADR had to change. Reports and per-spike Phase 07 carry-forward lists: `docs/architecture/spikes/README.md`. Open items carried into Phase 07: the public r2.dev URL (ADR-003 compliance), the cold first request re-measure at hosting (D-248), the EU bucket jurisdiction before real personnel files (D-249), and the QTE→PRJ sync-versus-event inconsistency between MODULE_BOUNDARIES and MODULE_MAP.
- **Validation finding (2026-09-20):** D-247 / CHG-007 adopted and folded. TASK-0091 one-call search: 18 x 20 samples max 190 ms; 19 additional request/security checks passed. Diagnostics retain 392/529/636 ms failures and now 540 ms on a verified new backend (465 ms server). A separate new-backend profile was fast, so startup alone does not explain the failure. Replay gate FAIL (OQ-029); equivalent-wrapper profile reproduced 432/354 ms across multiple stages; plan modes did not resolve it. Cold original wait sampling captured 571/487 ms with no sampled PG wait. A private exact-word B-tree/range candidate passed 24 parity/security checks; cold and independent source validation remain before adoption. Region move is postponed by the owner (DEF-009). Eight spikes complete; search remains REVIEW before SPIKE-14. No criterion waived. **2026-09-21:** cold triage placed the first-request cost outside the query (first touch of cached-looking pages after idle); the owner granted a cold-start exception (D-248 / CHG-008), warm searches stay under 300 ms (max 190 ms). SPIKE-09 through SPIKE-16 closed; the candidate "architecture boundary enforcement tooling", never turned into a spike, was tested at the owner's choice as SPIKE-17 and passed. Seventeen spikes complete; every candidate is covered; awaiting the owner's exit decision. The test project is on the Supabase Free Plan; the `spike` schema was dropped with owner approval after it exceeded the 500 MB limit.
...
- **Status:** `IN_PROGRESS` — **current phase since 2026-09-21** (Phase 06 exit approved). First step: the mandatory question round and implementation plans before any Phase 07 code (PROJECT_RULES §3, §10). Earlier partial delivery, kept as the record of what is owed:  Delivered ahead of the phase by M0 and CHG-004: scaffold, lint/boundary rules and test setup (TASK-0023), real Supabase authentication with TOTP 2FA and session guard (TASK-0024, TASK-0025), app shell with navigation registry, theme and brand tokens, role onboarding (TASK-0026, TASK-0032…TASK-0036). **Delivered since then (2026-09-21…24):** CI pipeline (TASK-0100), data access with migrations and the restricted runtime role (TASK-0101), dynamic roles / delegation / acting role / visibility (TASK-0102), audit & history (TASK-0103), outbox & jobs (TASK-0104), catalogs, dated rules and custom fields (TASK-0105), calendar and exchange rates (TASK-0106), documents & storage (TASK-0107), notifications & tasks core (TASK-0108), revision-request core (TASK-0109), site-wide search foundation (TASK-0110), the Home Screen install and phone targets (TASK-0113), the app card's functional bottom band (TASK-0028) and account security — recovery codes, sign-in lock, session rules (TASK-0112).

**Still owed (2026-09-24):** M1 itself (TASK-0111), which is the owner's own walk; its eleven steps are written down in `docs/features/m1-local-acceptance.md`. Everything else that was owed has been done: the browser pass covered the bottom band and account security on 2026-09-24 and fixed the four faults it found (a lock announcing a minute too many, a recovery placeholder that read like a real code, a heading sliding under the logo, and a people table whose only button fell off the edge of a narrow window), and the "Kullanıcılar & Roller" screen the manager's reset needed was built as a minimal one at the owner's choice (D-273). Walking the acceptance list before handing it over found two more, both now fixed: the "Revizyon Talepleri" menu entry opened a placeholder for a screen built with TASK-0109, and the account-security events reached the audit log with no Turkish name. TASK-0113's remaining acceptance — installing on a phone's Home Screen and receiving a push — waits for a real address after the owner declined a tunnel (D-274), and does not block M1 or Phase 08. Search keeps two acceptance items that belong to Phase 09: real module sources in a browser, and ranking when very many records share one word. Environments and a hosted deploy stay deferred (DEF-008).
...
- **Acceptance:** foundation features pass T1 gates; M1 review done locally with owner feedback recorded (D-245); hosted deployment and rollback are verified before the Slice 1 pilot (DEF-008).
...
- **Writing:** limited by D-268 to **drafts only** — the channel prepares a draft and nothing takes effect until the person opens it in the panel and confirms it; each draft carries an idempotency key and an audit entry marked `channel='mcp'`. Creating a task, adding a note or opening a revision request directly is not on the surface; approvals and money, stock and personnel records stay in the panel.
