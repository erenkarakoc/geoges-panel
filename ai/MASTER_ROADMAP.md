...
**Still owed:** the browser and real-device verification of the last three (the bottom band, account security, the Home Screen install), the "Kullanıcılar & Roller" screen that the manager's second-factor reset needs a button on — Phase 02 never specified it — and M1 itself (TASK-0111). Search keeps two acceptance items that belong to Phase 09: real module sources in a browser, and ranking when very many records share one word. Environments and a hosted deploy stay deferred (DEF-008).
