...
| ID | Decision | Ref |
|---|---|---|
| D-255 | **No pre-migration dump during the test period.** The owner: "we are still testing; we do not need backups; change the rule". Until the environment is marked as holding real data (`npm run db:mark-real-data`), migrations and rollbacks run without a dump; weekly manual dumps are optional too. From the mark on, the migration runner again refuses to run without a dump younger than an hour. The pre-real-data backup requirements (BACKUP_AND_RECOVERY section 3, DEF-008) are unchanged. Supersedes the local-period dump rule of TASK-0077. | Owner 2026-09-22; TASK-0076, TASK-0077, TASK-0101 |
