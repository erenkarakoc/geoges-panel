...
| D-220 | **Daily site log entry: stepped by default, section list on request.** The 15 sections come one by one with back/next and a summary step; the user can switch to a section list showing each section's state. A day strip at the top moves between days, coloured by state. Derived consequence, to be confirmed with the screen spec: the chosen view is remembered per user. Closes OQ-027 item 3; item 4 (notification-driven navigation) is carried by REQ-TSK-007 and REQ-TSK-008, so OQ-027 is closed | Owner 2026-09-19; REQ-SIT-003, REQ-SIT-035; OQ-027 |
...
| D-216 | **Slice order confirmed as planned** (phases 09–18, with 09R after the first pilot). **Every slice pilots on sample data only** (AI recommended a real site after the sample run); real use starts with the company-wide rollout in Phase 19, and the KVKK check of D-214 falls before that. **Pilot site and pilot users are named before slice 1's build ends** — an exit condition of Phase 09 | Owner 2026-09-19; D-048; ADR-007 |
