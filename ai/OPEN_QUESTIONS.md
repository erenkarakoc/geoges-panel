...
|---|---|---|---|---|
| OQ-029 | Architecture validation / owner decision | OPEN — now a DECISION, not a diagnosis: how should the 300 ms first-request target be met when the cost is outside the query? D-247 model and semantics remain adopted. | 2026-09-21 cold triage after ~10 h idle: `select 1` 0.08 ms and pure CPU 219 ms with no cold penalty, but a 2705-block scan counted as shared-buffer hits with zero reads took 517 ms first vs 58 ms after (~170 µs per block), and the search 383 ms first vs 40 ms. The penalty attaches to the first touch of data pages PostgreSQL believes are cached; most likely host memory reclaim during idle, not directly observable from PostgreSQL (verify with the Supabase memory/swap graph). Query design cannot fix it — index, plan mode, range rewrite and the one-call wrapper were all measured. Options for the owner: (a) keep-warm job that periodically touches the search relations, to be validated by a multi-hour experiment; (b) a larger compute tier, part of the hosting decision (DEF-008); (c) an explicit, recorded cold-start exception — only the owner may choose this. Earlier failures 392/529/540/554/571/636 ms and the new 462 ms are retained; replay FAIL. Region move postponed (DEF-009). See `docs/architecture/spikes/SPIKE-12-search-retry.md`. No waiver. | Product search implementation and Phase 06 exit until decided and validated |
