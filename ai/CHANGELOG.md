...
- CI 35832296803 caught a pagination defect after rollback/reapply: ORDER BY id selected the text output alias. The fix orders by the qualified bigint column; a new passing regression covers digit boundaries and adjacent IDs above JavaScript safe integer precision (32 search tests now). Fix CI pending.
