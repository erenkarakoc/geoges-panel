...
- Verification: 61 search DB tests (5 new), 203 database tests across 15 files, 273 unit tests, format and build pass; the 0030 down/up round trip leaves the answer identical. Warm smoke on the 56-row fixture 279 ms p95 / 284 ms max, against 271/272 before the check — roughly 10 ms for the check on this fixture, still inside the 300 ms target. CI 35867217424 green: 201 DB tests including all 61 search tests before and after a full rollback and reapply.
